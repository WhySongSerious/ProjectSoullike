...
namespace ProjectSoullike.Editor
{
    ...
    public static class SoullikeCombatAnimationSetup
    {
        ...
        private const string SourcePackageRelativePath =
            "SourceAssets/TC_Sword_Free_Pack/Unity_Pack/Unity_TC_Sword_Free_Pack.unitypackage";

        private const string ImportedAnimationFolder = "Assets/ThirdParty/TC_Sword/Animation";
        ...
        private static void BeginSetup(bool forceRebuild)
        {
            ...
            string workspaceRoot = projectRoot == null
                ? string.Empty
                : Directory.GetParent(projectRoot)?.FullName;
            string packagePath = string.IsNullOrEmpty(workspaceRoot)
                ? string.Empty
                : Path.Combine(
                    workspaceRoot,
                    SourcePackageRelativePath.Replace('/', Path.DirectorySeparatorChar));

            if (string.IsNullOrEmpty(packagePath) || !File.Exists(packagePath))
            {
                Debug.LogError($"{LogPrefix} Package not found: {SourcePackageRelativePath}");
                return;
            }
